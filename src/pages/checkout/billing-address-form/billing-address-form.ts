...
export class BillingAddressForm implements OnInit{
    ...
    order: any;
    chosen_shipping: any;
    constructor(public iab: InAppBrowser, public nav: NavController, public _service: Service, public loadingCtrl: LoadingController,public service: CheckoutService, params: NavParams, public functions: Functions, public values: Values) {
...
/*     setDataAddress() {
        ...
        //console.log(this._service.getAddress());   
        this._service.getAddress().then(
            result => {
                var customer = result['customer'];
                if(customer) {
                    this.form.billing_email = customer.email;
                    this.form.billing_first_name = customer.first_name;
                    this.form.billing_last_name = customer.last_name;
                    this.form.billing_document = "";
                }
                loading.dismiss();
            }
    ...
    checkout() {
        ...
        if (this.form.shipping) {
            ...
            this.form.shipping_document = this.form.billing_document; //cedula
        }
         var patron = /^[0-9]+$/;
        if(this.form.billing_document.search(patron)){
            this.functions.showAlert("ERROR", "Por favor, ingrese una cédula válida");
            this.buttonSubmit = false;
            return false;
        }
        if (this.form.payment_method == 'bacs' || this.form.payment_method == 'cheque' || this.form.payment_method == 'cod' || this.form.payment_method == 'Boton123Pago') {
            localStorage.setItem('CI',this.form.shipping_document);
    ...
    login() {
        if (this.validateForm()) {
            ...
            this.buttonText2 = "Iniciando sesión...";
    ...
    handleResults(a) {
        ...
        if (a.user_logged) {
            ...
            this.functions.showAlert("Error", 'Inicio de sesión incorrecto, intente nuevamente');
        }
    }
    submitCoupon() {
