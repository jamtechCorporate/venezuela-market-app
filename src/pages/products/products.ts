...
export class ProductsPage {
    ...
    sort: number = 0;
    categoryName: any;
    constructor(public nav: NavController, public popoverCtrl: PopoverController, public service: CategoryService, params: NavParams, public values: Values, public functions: Functions, public platform:Platform) {
        this.data = {};
        this.filter = {};
        this.q = "";
    ...
    updateToCart(id) {
        var items = this.products.products
        for (let i = 0; i < items.length; i++) {
            if(items[i].id === id && this.values.cart[id] < items[i].stock_quantity) {
                this.service.updateToCart(id).then((results) => this.status = results);
                break;
            } else {
                if(items[i].id === id && items[i].stock_quantity == null) {
                    this.service.updateToCart(id).then((results) => this.status = results);
                    break;
                }
            }
    ...
    chnageFilter(sort) {
        ...
        if (sort == 0) {
            ...
            this.filter['filter[orderby]'] = "menu_order";
        }
        if (sort == 1) {
            this.filter['filter[order]'] = "ASC";
