...
export class Orders {
    ...
    filter: any;
    status: any;
    constructor(public nav: NavController, public service: Service, public values: Values, public functions: Functions, public platform:Platform) {
        this.filter = {};
        this.filter.page = 1;
        this.count = 10;
        this.offset = 0;
    ...
    handleCancelOrder(results) {
        this.functions.showAlert("", "¡Su pedido ha sido cancelado con éxito!");
        this.service.getOrders(this.filter)
            .then((results) => this.orders = results);
    }
    reOrder(id) {
