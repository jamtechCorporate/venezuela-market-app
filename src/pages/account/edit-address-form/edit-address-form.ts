...
export class EditAddressForm {
    ...
    Save: any;
    constructor(public nav: NavController, public service: Service, params: NavParams, public values: Values, public functions: Functions) {
        this.Save = "Save";
        this.editAddress = params.data.item;
    ...
    saveAddress() {
        ...
        var patron = /^[a-zA-Z\s]*$/;
        if(this.editAddress.billing_address.first_name.search(patron)){
            this.functions.showAlert("ERROR", "No se acepta acentos ni la letra ñ en el campo nombre");
            this.disableSubmit = false;
            return false;
        }
        if(this.editAddress.billing_address.last_name.search(patron)){
