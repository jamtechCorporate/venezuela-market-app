...
export class ProductPage {
    ...
    validateStock() {
        console.log(this.product.product,this.values.cart);
        
        if( this.product.product.stock_quantity && this.values.cart[this.id]) {
            if((this.values.cart[this.id]+1) < this.product.product.stock_quantity) {
                return true;
            } else {
                return false;
            }
